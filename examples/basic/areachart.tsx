#!/usr/bin/env node
/**
 * AreaChart component example: Comparing two renderer charsets (Braille / Unicode Blocks)
 *
 * Optional parameters:
...
import { AreaChart, InkHudProvider, Legend } from '../../src/index';
...
const App = () => (
    <Box flexDirection="column" padding={1} gap={1}>
        <Text bold underline color="blue">
            AreaChart：Comparing charsets (renderers)
        </Text>
        <Text dimColor>
        ...
        <Box flexDirection="row" gap={2} flexWrap="wrap">
            {renderersToShow.map(({ label, rendererOverride, symbolSetId }) => {
                ...
                return (
                    <InkHudProvider key={label} renderers={rendererOverride ? { area: rendererOverride } : undefined}>
                        <Box flexDirection="column" borderStyle="round" padding={1} width={42}>
