#!/usr/bin/env node
/**
 * LineChart component example: Comparing two renderer charsets (Braille / Unicode Blocks)
 *
 * Optional parameters:
...
import { InkHudProvider, Legend, LineChart } from '../../src/index';
...
const App = () => (
    <Box flexDirection="column" padding={1} gap={1}>
        <Text bold underline color="blue">
            LineChart：Comparing charsets (renderers)
        </Text>
        <Text dimColor>
        ...
        <Box flexDirection="row" gap={2} flexWrap="wrap">
            {renderersToShow.map(({ label, rendererOverride, symbolSetId }) => {
                ...
                return (
                    <InkHudProvider key={label} renderers={rendererOverride ? { line: rendererOverride } : undefined}>
