...
describe('Axis', () => {
    describe('X Axis', () => {
        it('should render tick labels', () => {
            const { lastFrame } = render(<Axis type="x" min={0} max={100} length={50} />);
        ...
        it('should only show one tick when min === max', () => {
            const { lastFrame } = render(<Axis type="x" min={50} max={50} length={30} />);
            const output = stripAnsi(lastFrame() ?? '');
        ...
        it('should support custom tickFormat', () => {
            const { lastFrame } = render(
                <Axis type="x" min={0} max={100} length={50} tickFormat={(v) => `${v}%`} />,
            );
            const output = stripAnsi(lastFrame() ?? '');
            expect(output).toContain('0%');
            expect(output).toContain('100%');
        ...
        it('should force integer ticks when integerScale is true', () => {
            const { lastFrame } = render(
                <Axis type="x" min={0} max={3} length={30} tickCount={5} integerScale={true} />,
            );
            const output = stripAnsi(lastFrame() ?? '');
    ...
    describe('Y Axis', () => {
        it('should render tick labels', () => {
            const { lastFrame } = render(<Axis type="y" min={0} max={100} length={10} />);
            const output = stripAnsi(lastFrame() ?? '');
            expect(output).toContain('0');
            expect(output).toContain('100');
        });
